...
## 数值验证（headless 模拟，5 种子）

- 全部种子在 45–55 游戏日内**铸出青铜斧**（石器→青铜全程）
- 露天烧陶失败率 75%：首烧几乎必败 → 见识提示温度不足 → 解锁泥壳窑 → 窑烧 8% 失败（5/5 种子验证）
- 添丁成本随人口递增，棚屋同步扩容；研究岗分配保底，防死锁
- 远征收益：50% 保底区域特产（铜/锡可得性验证）
